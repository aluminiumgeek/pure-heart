...
var vkUtils = {
    groupsWallGet: function($scope, groups, uid, i) {
        ...
        if (i < groups.length && !stopped) {
            ...
            VK.Api.call('wall.get', params, function(result) {
                 if (result.response) {
                   vkUtils.likesChecker($scope, result.response, uid, 0, i, groups);
                }
            });
        }
        else {
    ...
    likesChecker: function($scope, posts, uid, i, parentIndex, parentCollection) {
        if (i < posts.length - 1 && !stopped) {
                ...
                VK.Api.call('likes.getList', params, function(liked) {
                    ...
                    setTimeout(
                        function() {
                            ...
                            var index = i + 1;
                            vkUtils.likesChecker($scope, posts, uid, index, parentIndex, parentCollection)
                        },
                        250
                    ); 
                });            
            ...
            parentIndex++;
            vkUtils.groupsWallGet($scope, parentCollection, uid, parentIndex);
        }
    }
}
